...
function PostItem({ img, comment, name, id }) {
    ...
    const { postDatas, setSavedData, savedData } = useContext(ContextData)
    ...
    const save = () => {
        ...
        if (!kayd) {
            setSavedData(st => [...st, {...findSave ,save:kayd}]);
        }
        else {
            const filterDatas = savedData.filter(item => item.id !== id)
            setSavedData(filterDatas)
            
        }
        console.log(savedData,"2jhkhjkhj");
    }
